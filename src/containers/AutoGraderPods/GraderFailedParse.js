...
class GraderFailedConnections extends Component {
  ...
  handleReplyClick = (obj, username) => {
    ...
    if (job_count === 0) {
      this.props.dispatch({
        ...
        data: {key: 'failed_parse', username}
      })
  ...
  handleReplyAllClick = () => {
    const { node_commands, lab, folder_name, job_count, failed_parse } = this.props;

    if (job_count === 0) {
      this.props.dispatch({
        type: C.REMOVE_ALL_FAILED_ENTITY,
        data: {key: 'failed_parse'}
      })
      ...
      let arr = []
      _.map(failed_parse, (value, key) => arr.push({[key]:value}))
      console.log(arr)
  ...
  render() {
    const { failed_parse} = this.props;
    return (
      <Col xs="12" lg="6">
            <Card>
              <CardHeader className='d-flex bg-secondary'>
                <i className="cui-thumb-down icons font-xl d-block"> </i>
                Failed Parse
                { !_.isEmpty(failed_parse) ?
              ...
              <CardBody>
                <Table hover bordered striped responsive size="sm">
                  ...
                  <tbody>
                  { _.map(failed_parse, this.renderData)}
                  </tbody>
                </Table>
              </CardBody>
            </Card>
...
const mapStateToProps = state => (
  {
    failed_parse: state.labgraderPods.grader_report.failed_parse,
