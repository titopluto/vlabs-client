...
class GraderFailedPasswords extends Component {
  ...
  handleReplyClick = (obj, username) => {
    ...
    if (job_count === 0) {
      this.props.dispatch({
        ...
        data: {key: 'failed_passwords', username}
      })
  ...
  handleReplyAllClick = () => {
    const { node_commands, lab, folder_name, job_count, failed_passwords } = this.props;

    if (job_count === 0) {
      this.props.dispatch({
        type: C.REMOVE_ALL_FAILED_ENTITY,
        data: {key: 'failed_passwords'}
      })
      ...
      let arr = []
      _.map(failed_passwords, (value, key) => arr.push({[key]:value}))
      console.log(arr)
  ...
  render() {
    const { failed_passwords} = this.props;
    return (
      <Col xs="12" lg="6">
            <Card>
              <CardHeader className='d-flex bg-secondary'>
                <i className="cui-lock-unlocked icons font-xl d-block"> </i>
                Failed Passwords
                { !_.isEmpty(failed_passwords) ?
              ...
              <CardBody>
                <Table hover bordered striped responsive size="sm">
                  ...
                  <tbody>
                  { _.map(failed_passwords, this.renderData)}
                  </tbody>
                </Table>
              </CardBody>
            </Card>
...
const mapStateToProps = state => (
  {
    failed_passwords: state.labgraderPods.grader_report.failed_passwords,
    node_commands: state.graderWhiteBoard.node_commands_pods,
    folder_name: state.labgraderPods.folder_name,
    lab: state.labgraderPods.lab_name,
...
export default connect(mapStateToProps, mapDispatchToProps)(GraderFailedPasswords);
