...
const labGraderPodsReducer = (state=initialState, action) => {
    switch (action.type) {
      ...
      case C.SEND_NOTIFICATION:
        if (action.data.reducerType === "labgraderPods") {
      ...
      case C.SET_POD_DEVICES:
          if (action.data.reducerType === 'labgraderPods') {
            return Object.assign({}, state, {pod_devices: action.data.msg});
          }
          else return state


      // case WEBSOCKET_OPEN:
      //   if (action.source === "autograder_pods") {
      ...
      case WEBSOCKET_MESSAGE:
        ...
        if (json_data['destination'] === "autograder_pods") {
...
export default labGraderPodsReducer
